...
void enableRawMode() {
  ...
  raw.c_cc[VMIN] =
      0;  // set minimum number of bytes of input needed before
...
void editorInsertRow(int at, char *s, size_t len) {
  ...
  E.row =
      realloc(E.row,
              sizeof(erow) *
                  (E.numrows + 1));  // reallocate the memory to store new row
...
char *editorPrompt(char *prompt, void (*callback)(char *, int)) {
  ...
  while (1) {
    ...
    if (c == '\x1b') {
      ...
      if (buflen ==
          bufsize - 1) {  // if the buflen reached the maximum capacity
